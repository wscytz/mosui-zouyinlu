...
| P1 | ~~Boss阶段提示文字~~ ✅ | 已实现：狂暴/绝望触发"狂 暴"/"绝 望"全屏火焰大字(phase reason) |
| P1 | ~~低血量警告增强~~ ✅ | 已实现：血量≤阈值时红色脉冲光环(player周)+屏幕红色vignette警告 |
| P1 | ~~结算评级动画~~ ✅ | 已实现：game.css gradeReveal关键帧(scale弹入)已在end-grade元素生效 |
| P1 | ~~波次预告增强~~ ✅ | 已实现：game.js:4355-4361 波次弹窗显示下一波+难度+威胁等级，Boss波显示"决战" |
