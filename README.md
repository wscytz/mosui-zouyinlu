...
- **输入契约** — `buildInputFrame()` 暴露为 `MOSUI.input.buildFrame`，便于后续调试、录像、控制器适配。
- **测试** — 187项（37冒烟+5波次+135内容+10压力）

## v3.1 (2026-05-07)

- **移动端操控重做** — 安全区补偿修正、摇杆上移+缩小、非线性三次方响应曲线、死区10→22。
- **自动瞄准+自动攻击** — 左手移动时系统自动锁敌攻击，右摇杆手动接管，松手后回自动。
- **锁敌黏性** — 锁定目标不跳，新目标需近35%+才切换，角度平滑插值。
- **触控反馈** — 摇杆激活高亮+方向线、自动瞄准红箭头指示、闪避/摇杆触觉震动。
- **测试** — 186项（37冒烟+5波次+134内容+10压力）

## v3.0.3 热修 (2026-05-07)

- **移动端按钮** — 闪避/暂停按钮改成更明确的触控样式，补 `aria-label`，降低误触感。
- **移动 HUD** — 重新留出一点顶部空间，减少按钮和状态条互相挤压。
- **测试** — 163项（37冒烟+5波次+111内容+10压力）

## v3.0.2 热修 (2026-05-07)

- **移动端诊断** — debug 面板显示移动UI、摇杆、瞄准和闪避状态，方便真机排查。
- **移动输入收口** — 右摇杆松手后保留最后瞄准方向，闪避按钮改为一次性请求。
- **测试** — 162项（37冒烟+5波次+110内容+10压力）

## v3.0.1 热修 (2026-05-07)

- **移动端输入隔离** — APK/`?mobile=1` 才初始化摇杆，网页端不再被 `__forceMobileInit()` 误切到移动输入。
- **移动按钮隔离** — `.mobile-btn` 只在 `body.is-mobile-ui` 下显示，不再靠触摸特征影响网页端。
- **测试** — 161项（37冒烟+5波次+109内容+10压力）
