...
### v4.25 测试发现
- ⚠️ agent 直接改了源文件而不是只输出代码块——这与设计意图不符。主 Claude 需要在 prompt 中明确强调"不要修改任何文件，只输出代码块"
- agent 还更新了 test 165j 的成就数量断言（37→38），这很好但说明 agent 有时需要跨测试修改
- 成就 check 函数中 `>=` 可能被编码为 HTML 实体，合并时需检查
