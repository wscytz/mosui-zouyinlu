...
这套系统的核心不是“让 agent 更聪明”，而是把聪明限制在可验证的轨道里。agent 负责想内容和填模板，主 Claude 负责事实、合并和测试，脚本负责发号、门禁、巡检。能交给脚本的规则不要交给记忆，能用 fixture 固化的踩坑不要只写进聊天。

内容生产按工厂思路运行：`skill` 是说明书，`ctx` 是事实源，`sequencer` 是排队叫号机，`executor` 是代工位，`rules/validator` 是门卫，`merger` 是入库员，`audit` 是体检表。任何一个角色都不要越权；越权会省一两分钟，但会把并发成本、测试编号、插入点和风格漂移重新推回人工。

长期目标不是无限堆遗物数量，而是让内容有结构：新内容要能进入构筑线、权重表、测试和视觉表达。每一波批量生产之后都要做治理：补 CSS 图标、补 RELIC_RULES、补冷标签、删重复想法、把复发错误转成门禁。
